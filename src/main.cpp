...
#define IMPULSE_Y 1000000
#define IMPULSE_X 1000000000
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			if (event.type == sf::Event::KeyPressed)
			{
				...
				{
					if(squareBody->GetLinearVelocity().y<=0.3f)
					delta_move.y -= IMPULSE_Y^1000;
				}
				
				if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left))
				{
					if(squareBody->GetPosition().x>borderLeft.getPosition().x)
						delta_move.x -= IMPULSE_X ^1000;
			...
			{
				if(squareBody->GetLinearVelocity().x>=0.3|| squareBody->GetLinearVelocity().x <= -0.3)
				squareBody->SetLinearVelocity(b2Vec2::b2Vec2(0.f, squareBody->GetLinearVelocity().y));

				if (squareBody->GetLinearVelocity().y >= 0.3 || squareBody->GetLinearVelocity().y <= -0.3)
					squareBody->SetLinearVelocity(b2Vec2::b2Vec2(squareBody->GetLinearVelocity().x, 0.f));
			}
		...
		if (delta_move.x!=0)
			squareBody->SetLinearVelocity(b2Vec2::b2Vec2(delta_move.x, squareBody->GetLinearVelocity().y));
